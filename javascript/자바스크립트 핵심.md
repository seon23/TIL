...
- font-face 로딩, DOM ApI 등이 Promise를 통해 구현된다.
- Promise는 주로 **비동기적인 로직을 하나의 값으로 처리**할 때 사용 된다.


### 동기적, 비동기적

#### 자바스크립트에서의 비동기
- 자바스크립트 실행 환경인 Node.js나 브라우저는 single thread, nonblocking 성질을 가진다.
- I/O bound 작업 등은 작업순서가 순차적으로 완료되지 않고 비동기적으로 일어난다. 

#### 동기적(synchronous)
- 일 A, B, C가 순차적으로 처리되는 것
- 일B 처리 시간이 길면 일C는 계속 기다려야 함(blocking)
    - 사이트에서 렉이 걸리는 이유
        - 이를 방지하고자 브라우저는 일반적으로 많은 시간이 소모되는 <u>I/O bound 작업과 web API의 setTimeout 등의 작업</u>은 비동기적으로 실행되도록 구축함

#### 비동기적(asynchronous) 
- 일 A, B, C가 순차적으로 발생하지만, 동시에 실행되며 일이 완료되는 순서대로 나타난다.
- 실행 완료시점의 순서가 보장되지 않는다. 

### I/O Bound, CPU Bound 작업
#### I/O Bound
파일시스템, 네트워크 등을 다루는 작업

#### CPU Bound 작업
CPU 자원을 사용하는 일을 다루는 작업

### 비동기적으로 작동되는 setTimeout
```
const job_1 = () => {
...
job_3()
```
- 위 함수는 랜덤 시간 후에 각각 1, 2, 3을 출력하는 함수이다.
- 함수를 순차적으로 호출하지만, 실행할 때마다 함수가 다른 순서대로 완료된다. 이는 setTimeout이 비동기적으로 작동하기 때문이다.
...
*/
```
Hi Javascript가 먼저 출력되는 이유
- 기본적으로 자바스크립트의 함수는 콜스택(Call Stack)에 쌓였다가 pop이 되면서 동기적으로 실행된다.
- 하지만 web APIs의 setTimeout 함수, I/O bound 작업은 콜스택에 쌓인 다음 바로 web APIs의 백그라운드 공간으로 넘어간 후 비동기적으로 완료가 되면 콜백큐(Callback queue)에 쌓이고 이벤트 루프(Event Loop)를 이용해 다시 콜스택에 쌓여 호출된다. 
